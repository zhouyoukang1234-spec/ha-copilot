...
| 原语 | 作用 |
|---|---|
| `query_entities` | **一术演万法**：通用实体查询，按 `domain` + 名字子串（匹配 entity_id 或友好名）+ `device_class` + 精确 `state` + `attributes` 过滤，`match` 控制 名字/类别 取 OR(any) 还是 AND(all)；无任何过滤则返回域内全部。**等价于数百个固定的 `*_check`/`*_status`/`*_monitor` 扫描-过滤工具**——优先用它而非具体的特化工具 |
